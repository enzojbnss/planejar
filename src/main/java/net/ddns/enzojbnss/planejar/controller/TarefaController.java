...
import net.ddns.enzojbnss.planejar.util.CompareData;
...
public class TarefaController {
	...
	public void salvar(Tarefa tarefa, String inicio, String fim, Long idProjeto) {
		...
		try {
			...
			if (CompareData.validaDataInicial(projeto.getInicio(), tarefa.getInicio())
					&& CompareData.validaDataFinal(projeto.getFim(), tarefa.getFim())) {
				if (tarefa.getId() == 0) {
					tarefa.setId(null);
				...
				teste = new TesteExecute(false, "Data da Tarefa fora do prazo do projeto");
			}
		} catch (Exception e) {
	...
	public void excluir(Long idTarefa) {
		...
		if (teste.isStatus() == true)
			teste.setMensagem("Tarefa excluidacom sucesso");
		this.result.use(Results.json()).withoutRoot().from(teste).serialize();
	}
}
